...
namespace BankAccountAPITest.Controllers.AccountControllerTests
{
    public class WithdrawTests
    {
        ...
        public async Task WhenUserNotfound_ShouldThrowException()
        {
            ...
            var exception = await Record.ExceptionAsync(async () => await sut.Withdraw("name", new TransactionContract(){AccountNumber = accountNumber, Amount = 10}));
        ...
        public async Task WhenTransactionAmountLessThan0_ShouldThrowException()
        {
            ...
            var exception = await Record.ExceptionAsync(async () => await sut.Withdraw(username, new TransactionContract(){AccountNumber = accountNumber, Amount = 0}));
        ...
        public async Task WhenAccountNotFound_ShouldThrowException()
        {
            ...
            var exception = await Record.ExceptionAsync(async () => await sut.Withdraw(username, new TransactionContract(){AccountNumber = accountNumber, Amount = 10}));
            Assert.IsType<ItemNotFoundException>(exception);
            Assert.Contains("Account not found.", exception.Message);
        } 
        
        [Fact]
        public async Task WhenBalanceIsInsufficient_ShouldThrowException()
        {
            var options = new DbContextOptionsBuilder<BankContext>()
                .UseInMemoryDatabase(Guid.NewGuid().ToString())
                .Options;
            ...
            var context = new BankContext(options);
            context.Users.Add(new User() {Username = username});
            context.Accounts.Add(new Account() {Username = username, AccountNumber = accountNumber});
            await context.SaveChangesAsync();
            var sut = new AccountController(context, _loggerMock.Object);
        ...
        [Fact]
        public async Task WhenBalanceIsSufficient_ShouldUpdateBalance()
        {
            var options = new DbContextOptionsBuilder<BankContext>()
                .UseInMemoryDatabase(Guid.NewGuid().ToString())
                .Options;
